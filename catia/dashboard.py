...
_REPORT_CACHE: Dict[str, Tuple[float, Optional[Dict[str, Any]]]] = {}
_REGISTER_CACHE: Dict[str, Tuple[float, Optional[Dict[str, Any]]]] = {}
...
def load_assumption_register(output_dir: Optional[str] = None) -> Optional[Dict[str, Any]]:
    ...
    if cached and cached[0] == mtime:
        return cached[1]
    if not path.exists():
        _REGISTER_CACHE[key] = (mtime, None)
        return None
    try:
        with open(path) as f:
            data = json.load(f)
        ...
        _REGISTER_CACHE[key] = (mtime, None)
        return None
    _REGISTER_CACHE[key] = (mtime, data)
    return data
    ...
    *,
    ...
    app.layout = html.Div(
        ...
        children=[
            ...
            dcc.Store(id="live-feed-store", storage_type="memory"),
                            ...
                            html.Div(
                                ...
                                children=[
                                    ...
                                    dcc.Dropdown(
                                        ...
                                        clearable=False,
                                        searchable=True,
                                        placeholder="Use latest run focal region",
                                        maxHeight=320,
                                        persistence=True,
                                        persistence_type="session",
                                        className="catia-dash-dropdown",
    ...
    def render_tab(active: str, _n: int):
        ...
        if triggered == "refresh-interval" and active != "tab-globe":
            raise PreventUpdate
        ...
        if active == "tab-live":
            return html.Div(
                ...
                children=html.Div(
                    className="catia-loading-placeholder",
                    children=html.P(
    ...
    def refresh_live_feed_store(_live_n: int, tab: str):
        if tab != "tab-live":
            raise PreventUpdate
        force = False
        if callback_context.triggered:
            force = callback_context.triggered[0]["prop_id"].startswith("dash-tabs")
        return fetch_live_events_base(force=force)
    ...
    def render_live_tab_content(
        ...
        report = load_report(str(out))
        return build_live_earth_content(
            store=store,
            report=report,
            peril_sel=peril_sel,
            min_score=min_score,
